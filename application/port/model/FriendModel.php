...
class FriendModel extends Model
{
	...
	public function friend_add($name,$phone,$openid)
	{
		...
		else
		{
			...
			$data = array(
					"friend_name"=>$name,
					"friend_phone"=>$phone,
					"friend_imgurl"=>"/images/img/tx".rand(0,10).".jpg",
					"time"=>time(),
					"uid"=> 0,
					"start"=>"0"
				);
	...
	public function MyFriend($openid)
	{
		$uid = $this->UserOpenid($openid);
		//查询我的好友
		$data = DB::name("user_friend")->alias("uf")->join("bill_friend bf","uf.u_id=bf.uid")->order("bf.f_id desc")->where("u_id",$uid)->select();
		// print_r($data);die;
